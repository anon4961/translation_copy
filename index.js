 ...
 *
 * 'keep'   - 원본 그대로
 * 'strip'  - <pre>/<code>/<script>/<style> 블록 DOM에서 제거 후 innerText 추출
 *            → 코드블록 제거, 나머지 텍스트 보존
 * 'remove' - 모든 요소 제거 후 innerText 추출 (텍스트만 남기되, 코드블록 내용도 제거)
 *            → 단, <status>/<choices> 같은 커스텀 태그는 innerText로 내용 보존
 *            → <pre>/<code>/<script>/<style>/<div>/<span> 등 순수 마크업 블록은 제거
 */
function applyHtmlModeToTranslation(text, htmlMode) {
    ...
    if (htmlMode === 'strip') {
        // <pre>, <code>, <script>, <style> 블록 통째로 제거 (코드블록 = 마크다운 ```...```)
        const removeSelectors = ['pre', 'code', 'script', 'style'];
        removeSelectors.forEach(sel => {
            tmp.querySelectorAll(sel).forEach(el => el.remove());
        });
        // {{img::...}} 제거
        let result = (tmp.innerText || tmp.textContent || '').replace(/\{\{img::[^}]*\}\}/gi, '');
        return result.trim() || result;
    }

    if (htmlMode === 'remove') {
        // <pre>, <code>, <script>, <style>, <div>, <span>, <p>, <br> 등 마크업 전용 블록 제거
        // 커스텀 태그(<status>, <choices> 등)는 innerText 보존
        const removeSelectors = ['pre', 'code', 'script', 'style', 'head', 'html', 'body', 'meta', 'link', 'title'];
        removeSelectors.forEach(sel => {
            tmp.querySelectorAll(sel).forEach(el => el.remove());
        });
        // 남은 일반 HTML 블록(div, span, p 등)은 innerText 추출
        let result = (tmp.innerText || tmp.textContent || '').replace(/\{\{img::[^}]*\}\}/gi, '');
        // 추출 후에도 남은 XML 스타일 커스텀 태그 블록 제거
        let prev;
        do {
            prev = result;
            result = result.replace(/<([A-Za-z][A-Za-z0-9]*)[^>]*>[\s\S]*?<\/\1>/g, '');
