 ...
 * llm-translator가 HTML로 렌더링해서 저장하므로
 * 브라우저 DOM 파싱(innerText)으로 처리해야 코드블록 내용이 올바르게 텍스트 추출됨
 * 'keep'   - 원본 그대로
 * 'strip'  - DOM 파싱으로 태그 제거, 텍스트 보존 + {{img::...}} 제거
 * 'remove' - DOM 파싱 후 남은 텍스트에서 추가로 XML태그 블록 제거
 */
function applyHtmlModeToTranslation(text, htmlMode) {
    if (!text) return null;
    if (htmlMode === 'keep') return text;

    // DOM 파싱으로 HTML 태그 제거, innerText만 추출
    const tmp = document.createElement('div');
    tmp.innerHTML = text;
    let result = tmp.innerText || tmp.textContent || text;

    // {{img::...}} 제거
    result = result.replace(/\{\{img::[^}]*\}\}/gi, '');

    if (htmlMode === 'remove') {
        // innerText 변환 후 남은 XML 스타일 태그 블록 제거
        let prev;
        do {
            prev = result;
            result = result.replace(/<([A-Za-z][A-Za-z0-9]*)[^>]*>[\s\S]*?<\/\1>/g, '');
