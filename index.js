 ...
 * 텍스트에 htmlMode 적용
 * 'keep'   - 원본 그대로
 * 'strip'  - 코드블록 제거 + HTML 태그만 제거(정규식), 텍스트 보존
 * 'remove' - 코드블록 제거 + XML/HTML 스타일 태그 블록 통째로 제거
 *
 * 번역문은 display_text 특성상 innerText 파싱을 쓰지 않고
 * 원문과 동일하게 정규식 기반으로 처리하여 일관성 확보
 */
function applyHtmlMode(text, htmlMode) {
    ...
    if (htmlMode === 'remove') {
        // XML/HTML 스타일 태그 블록(태그+내용) 반복 제거
        // <TagName ...>...</TagName> 형태를 중첩 포함해서 제거
        let prev;
        do {
            prev = result;
            result = result.replace(/<([A-Za-z][A-Za-z0-9]*)[^>]*>[\s\S]*?<\/\1>/g, '');
        } while (result !== prev);
        // 남은 단독 태그 제거 (<br>, <meta ...> 등)
        result = result.replace(/<[^>]+>/g, '');
        return result.trim() || null;
    }

    // 'strip' - 태그만 제거, 텍스트 보존 (정규식 방식, 원문/번역문 동일하게)
    // 코드블록 제거 후 남은 <tag ...> 형태를 모두 제거
    result = result.replace(/<[^>]+>/g, '');
