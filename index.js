...
function applyHtmlModeToTranslation(text, htmlMode) {
    ...
    // ── step 1: 정규식으로 커스텀 XML 태그 블록 제거 ──
    // <status>...</status>, <choices>...</choices>, <UpdateVariable>...</UpdateVariable> 등
    // 브라우저가 비표준 태그 내용을 innerText에 노출시키므로 DOM 파싱 전에 먼저 제거
    let cleaned = text;
    let prev;
    do {
        prev = cleaned;
        cleaned = cleaned.replace(/<([A-Za-z][A-Za-z0-9]*)[^>]*>[\s\S]*?<\/\1>/g, '');
    } while (cleaned !== prev);
    // 남은 단독 커스텀 태그 제거
    cleaned = cleaned.replace(/<[^>]+>/g, '');

    if (htmlMode === 'remove') {
        // 커스텀 태그 제거 후 남은 것도 전부 제거 → 순수 텍스트만
        // {{img::...}} 제거
        cleaned = cleaned.replace(/\{\{img::[^}]*\}\}/gi, '');
        return cleaned.trim() || null;
    }

    // 'strip': 커스텀 태그는 제거했지만, 원본 text에서 표준 HTML(pre/code 등) 처리도 필요
    // 원본을 DOM 파싱해서 pre/code/script/style 제거 후 innerText 추출
    const tmp = document.createElement('div');
    tmp.innerHTML = text;
    // 커스텀 태그 내용이 innerText에 노출되므로 정규식으로 처리한 cleaned 사용
    // 대신 표준 HTML 태그(br→줄바꿈 등)의 렌더링은 DOM에서 처리
    ['pre', 'code', 'script', 'style'].forEach(sel => {
        tmp.querySelectorAll(sel).forEach(el => el.remove());
    });
    // DOM의 innerText에서 커스텀 태그 내용이 남아있으므로
    // cleaned(커스텀 태그 제거 완료본)에서 표준 HTML 태그만 추가로 제거
    let result = cleaned.replace(/\{\{img::[^}]*\}\}/gi, '');
    return result.trim() || result;
}
